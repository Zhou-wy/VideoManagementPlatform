 ...
 * 保存摄像头配置
 * */
int addVideo::saveVideoListConf(QList<std::shared_ptr<VideoConf>> &videoList) {
    ...
    QString prevVideoName;
    if (videoList.length() != videoList.last()->video_name.split("video").last().toInt()) { //判断video1----video_i之间是否有缺项
        for (int i = 0; i < videoList.size(); i++) {
            QString currentVideoName = videoList.at(i)->video_name;
            if (!prevVideoName.isEmpty() &&
                currentVideoName.split("video").last().toInt() != prevVideoName.split("video").last().toInt() + 1) {
                ...
                if (m_type.toLower() == "rtsp") {
                    ...
                    QString Auxiliary_code_stream =
                            "rtsp://" + m_userName + ":" + m_passWord + "@" + m_ip + ":" + m_port +
                ...
                videoList.insert(i, newVideoConf);
                return i;
            }
            prevVideoName = currentVideoName;
        }
    } else {
