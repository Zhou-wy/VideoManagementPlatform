...
void SeViManPlat::initVideoPlay() {
    if (VideoConfJson->getString("VideoCount") == "") {
        ...
        QString url = "";
        int i = 0;
        for (const auto &conf: m_videoConf) { // 遍历所有的摄像头配置
...
void SeViManPlat::addVideoPlay() {
    this->addVideoTool->saveVideoListConf(m_videoConf);
...
void SeViManPlat::onTreeViewClickedStream(const QModelIndex &index) {
    ...
    if (!parentIndex.isValid()) {
        ...
        iDEBUG(ROOT_LOG) << "clicked the video: " << video_i << " len m_videoConf: " << m_videoConf.length();
