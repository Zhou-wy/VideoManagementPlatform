//
// Created by zhouwy on 2023/2/22.
//
...
namespace iLogger {
    ...
    vector<string>
    find_files(const string &directory, const string &filter, bool findDirectory, bool includeSubDirectory) {
        string realpath = directory;
        if (realpath.empty())
            realpath = "./";
        ...
        while (!ps.empty()) {
            ...
            if (handle not_eq 0) {
                while (fileinfo == readdir(handle)) {
                    ...
                    if ((!findDirectory && !S_ISDIR(file_stat.st_mode) )|| (findDirectory && S_ISDIR(file_stat.st_mode))) {
    ...
    string
    replace_string(const string &str, const string &token, const string &value, int nreplace, int *out_num_replace) {
